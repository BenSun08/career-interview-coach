...
Exercise original-page failure through the public link-report command: API detail returns 200, the owner reports the saved link unavailable, and the UI retains the separate reported status and timestamp through another successful API recheck. Clear the report explicitly and expect unknown page availability. Test stale report bases, unauthorized reports, changed source URLs and duplicate mutation IDs. Request capture must prove report/clear causes zero outbound requests; an opened external tab does not provide reliable page-status evidence.
...
Use controllable server and browser clocks and actual temporary files. Test before/at/after creation+24 hours; early text-and-review completion; interrupted/failed review; retry/re-entry; keep just before deletion; keep concurrent with deletion; late upload; transcode remnants; worker restart; missing file; purge permission failure. After upload failure, leave the tab open through expiry: capture stops, memory buffers/object URLs are discarded, playback/retry is disabled, and the UI says expired. Verify acknowledgement, cancellation and page teardown discard buffers/object URLs, and that audio never enters persistent browser caches. Resume a previously suspended tab after the deadline and verify expiry runs before any playback, retry or upload. Before capture, the UI must disclose that clearing unsaved memory during suspension is not guaranteed and offer text/cancel alternatives. These are the approved browser acceptance criteria; do not report them as proof of deletion while suspended. Original deadlines never move. Kept audio is exempt from temporary TTL; text/review defaults remain until deletion. Access expiry and actual file removal are separately asserted.
...
Restore tests must retain the current installation's authority separately from the candidate: take a registered backup, accept deletion, simulate failed backup purge, then attempt restore and expect rejection of that now-ineligible copy. Reject missing/corrupt current authority, unregistered/retired/tampered copies and newer unsupported schemas without replacing the live database. For an eligible older-schema backup, preserve current tombstones/generation/copy eligibility, apply expiry, invalidate sessions/cursors and cancel replayable jobs before serving. Interrupt before reconciliation and during replacement; startup must stay in maintenance or recover the preserved current database, never serve stale personal data. These are controlled-maintenance guarantees, not archive-only disaster recovery.

Repeat with post-backup claim rejection, an accepted content edit, consent revocation and a password change. Current authorization/privacy choices must remain effective; a candidate that would lose later accepted records or reinstate obsolete factual validity must be rejected rather than silently rolling those decisions back.
