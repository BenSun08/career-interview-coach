...
**Historical task-generation baseline**: `main`, HEAD `9895af7`, then ahead of `origin/main` by one commit. The 2026-09-25 consistency remediation reverified `main` at `16760b0`, with no application scaffold. Reverify before later Git work. Task generation and this remediation make no branch, commit, push or PR.
...
- `Depends` lists prerequisite tasks. All must be complete before starting that task; dependencies are transitive. Increasing IDs provide a safe serial execution order, but independent qualification branches may follow their explicit dependencies. Test authoring and executable RED verification are separate checkpoints, as defined below; an import error never counts as behavioral RED.
...
### Test authoring and executable RED

A test-only task first specifies and reviews the focused assertions, fixtures and public outcomes in its listed test files. Run any cases whose prerequisites exist. If an application signature, route or table is still absent, record that structural blocker and the named implementation task that will execute RED; the test-authoring checkbox records authorship only, never successful execution. Do not substitute mocks of the behavior under test or import errors for evidence.

At the start of each named implementation task, add only the minimum importable signature, explicit unimplemented route or unconstrained schema needed to execute its tests, within that task's own file list. Observe the intended assertion fail before implementing the corresponding rule, constraint, side effect or transition, then record GREEN for the same case. For schema tests, establish the predecessor/minimal table first, observe the missing constraint or preservation failure, and then add the migration change. This setup does not pre-authorize working business behavior. Shared implementation files remain serialized; parallel test authors do not create application stubs.

Each implementation task's completion requires its recorded RED-to-GREEN evidence and its original Done criterion. A test task can therefore precede its model without a dependency cycle. Cross-story regression tests in T105 run against already implemented behavior: record a fresh pass when it is correct, and obtain RED before repairing any newly exposed defect; do not deliberately break correct code to manufacture RED.
...
- [ ] T006 [P] Author focused access-boundary tests for owner/second-account/anonymous access, login expiry/throttling, CSRF, Origin/Host, private no-store responses and safe errors. Files: `tests/contract/test_access.py`. **Depends**: T005. **Basis**: FR-026, FR-029, FR-030; API Transport; Q1. **Test contract**: Failures identify absent owner protections, including private-file and operation-route contract cases. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T013, T018, T075, T099.

- [ ] T007 [P] Author focused real-connection transaction tests for revision membership, mutation replay/mismatch, overlapping candidates, scalar merge, ordered-content conflict, deleted-write rejection, cursor atomicity and persisted ReviewRequirement uniqueness/rollback/reload. Include forward/reverse schema checks with populated predecessor fixtures and the phase's declared model constraints. Files: `tests/integration/test_revisions.py`, `tests/migrations/test_core_schema.py`. **Depends**: T005. **Basis**: FR-002, FR-026; Data model Shared Rules/Concurrency; Q10. **Test contract**: Assertions use persisted outcomes, preserve both candidates and expose rollback/race failures. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T010, T014.

- [ ] T008 [P] Author focused tests for denied/revoked/provider-changed processing choices, job leases, one bounded transient retry and stale/deleted/unauthorized result publication. Files: `tests/integration/test_processing_jobs.py`. **Depends**: T005. **Basis**: FR-020, FR-030; API Operation Publication and Failure; Q1. **Test contract**: Request capture proves no sends for blocked choices and no double publication after retries. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T011, T012, T015, T017.

- [ ] T009 [P] Author focused storage/policy tests for owner-only files, registered copies, UTC deadlines, earlier completion/deletion triggers and visible purge failures with access revoked. Files: `tests/integration/test_private_storage.py`. **Depends**: T005. **Basis**: FR-027, FR-028; Interaction lifecycle Recording Retention; Q11. **Test contract**: Actual temporary files distinguish inaccessible, pending and physically removed states. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T011, T016.

### Models and migrations

- [ ] T010 Create concrete owner/device, mutation/conflict/change and explicit EvidenceReference/ReviewRequirement tables with an initial migration; apply shared ownership, revision and date constraints without introducing an arbitrary entity store. Files: `career_helper/models/core.py`, `career_helper/models/__init__.py`, `career_helper/migrations/0001_core.py`. **Depends**: T006, T007. **Basis**: FR-002, FR-026; Data model Shared Rules/Dependency Validity. **Done**: Migration enforces owner/revision/operation uniqueness and explicit artifact/dependency kinds and annotation indexes; core migration reverses on an empty fixture.
  ...
  > **EvidenceReference — fields**: owner, artifact_kind, artifact_revision_id, content_unit_id, dependency_kind, dependency_revision_id.
  >
  > **EvidenceReference — validation/lifecycle**: Unique owner/artifact/unit/dependency tuple; finite artifact/dependency kinds, reverse-dependency index, and exact same-owner revision membership validated in the write transaction. No copied personal text.

  > **ReviewRequirement — fields**: owner, artifact_kind, artifact_revision_id, content_unit_id, dependency_kind, dependency_revision_id, reason_code, discovered_at.
  >
  > **ReviewRequirement — validation/lifecycle**: Unique owner/artifact/unit/dependency/reason tuple; affected-artifact index; immutable content-free annotation. Acknowledgement cannot remove it; privacy deletion may purge it with its artifact.
...
- [ ] T011 Add the processing-choice, retention-policy, copy inventory, deletion and export operation tables; chain the migration after the core migration. Files: `career_helper/models/privacy.py`, `career_helper/models/__init__.py`, `career_helper/migrations/0002_privacy.py`. **Depends**: T010, T009, T008. **Basis**: FR-027, FR-028, FR-030; Data model Concurrency, Processing Choice, and Privacy. **Done**: Defaults, state constraints and prior-schema preservation pass focused migration assertions.
  ...
  > **StoredCopy — fields**: kind (`original`, `audio`, `transcode_temp`, `export`, `backup`), private location, content references, checksum, created_at, expires_at nullable, deletion state.
  >
  > **StoredCopy — validation/lifecycle**: Enumerate product-controlled copies; no temporary audio in export/backups; export expiry and backup retention visible. Backup eligibility is checked against current deletion requests, never an archived inventory alone.
...
- [ ] T021 [P] [US1] Author focused import contract tests and synthetic fixtures for readable PDF/DOCX, image-only/corrupt/oversized content, partial extraction, tracked changes, unsafe archives and embedded instructions. Files: `tests/contract/test_resume_imports.py`, `tests/fixtures/documents/build_fixtures.py`. **Depends**: T020. **Basis**: FR-001, FR-029; SC-010; Q2; External services Document Extraction. **Test contract**: Tests assert source order/locators, pending facts, diagnostics/manual fallback and bounded parser failure. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T024, T026, T027, T028.

- [ ] T022 [P] [US1] Author focused claim lifecycle tests for partial dates, unknown numbers, work versus learning/interest, explicit edit-and-confirm, conflicting sources and dependent-content invalidation, including persisted impact annotations across reload and populated upgrades. Include forward/reverse schema checks with populated predecessor fixtures and the phase's declared model constraints. Files: `tests/integration/test_claims.py`, `tests/e2e/profile.spec.js`, `tests/migrations/test_profile_schema.py`. **Depends**: T020. **Basis**: FR-001–FR-003; SC-002; Q2/Q3. **Test contract**: Current eligibility and immutable history assertions cover direct invalid inputs and transaction rollback. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T024, T025, T029, T030.

- [ ] T023 [P] [US1] Author focused catalog/filter/assessment tests across automotive, frontend and learning-only synthetic profiles, including all three stage-decision branches from Spec Acceptance Interpretations and Input Boundaries, essential unknowns, interest-only evidence and high-gap favorites. Files: `tests/unit/test_directions.py`, `tests/fixtures/profiles.json`. **Depends**: T020. **Basis**: FR-004–FR-006; SC-001; Q4. **Test contract**: Every profile expects three or more directions in each required domain without becoming multi-user support. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T025, T031.
...
- [ ] T025 [US1] Add preferences, career catalog and assessment revisions, initially scoped to directions; add the concrete target relationship in the US2 migration. Files: `career_helper/models/careers.py`, `career_helper/models/__init__.py`, `career_helper/migrations/0005_careers.py`. **Depends**: T024. **Basis**: FR-004–FR-006; Data model Targets, Sources, and Assessments. **Done**: Allowed stage values and stable ordered favorites are preserved across migration.
  ...
  > **CareerDirection — fields**: catalog_version, stable direction_id, domain, title, typical work, skill_requirements (essential/supporting, kind knowledge/practice/experience/credential), knowledge_topics.
  >
...
- [ ] T033 [US1] Build desktop experience review and direction screens with source/confirmation/conflict labels, editable preferences, evidence/gaps/unknowns and manual fallback. Files: `career_helper/templates/career_helper/profile.html`, `career_helper/templates/career_helper/directions.html`, `career_helper/static/career_helper/profile.js`. **Depends**: T032. **Basis**: FR-001–FR-006; UX frames 01/02/12A. **Done**: Q2/Q4 browser journey handles empty, partial and failure states with accessible feedback and the spec-defined next actions.
...
- [ ] T035 [P] [US2] Author focused source boundary tests for fixed approved requests, disallowed redirects/hosts, malformed/oversized replies, deadlines/retries and absence of personal query content. Files: `tests/contract/test_job_source.py`, `tests/fixtures/jobs/responses.json`. **Depends**: T034. **Basis**: FR-029–FR-035; External services Job Acquisition; Q5. **Test contract**: Captured requests contain only board/options; exact error/freshness outcomes are asserted. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T039, T041.

- [ ] T036 [P] [US2] Author focused aggregation/target tests for exact duplicates, two-source conflicts, unknown dates, location boundaries, high gaps, API-detail-success plus owner-reported broken original links, report/clear races and overseas manual descriptions. Include forward/reverse schema checks with populated predecessor fixtures and the phase's declared model constraints. Files: `tests/integration/test_listings_targets.py`, `tests/e2e/listings.spec.js`, `tests/migrations/test_targets_schema.py`. **Depends**: T034. **Basis**: FR-007, FR-031–FR-034; SC-004, SC-011, SC-012; Q5. **Test contract**: Report/clear sends no network traffic; detail recheck cannot clear the report; uncertain identity stays separate and selecting a variant never silently blends a job description. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T037, T038, T040, T042, T043, T044.

### Models and migrations

- [ ] T037 [US2] Add preview/search/fetch/observation/listing and ListingLinkReport tables and source identity constraints; preserve every collection occurrence and source variant. Files: `career_helper/models/listings.py`, `career_helper/models/__init__.py`, `career_helper/migrations/0006_listings.py`. **Depends**: T035, T036. **Basis**: FR-031–FR-035; Data model Targets, Sources, and Assessments. **Done**: Prior profile data survives migration and source-empty differs from local-no-match.
  ...
  > **ListingLinkReport — fields**: owner, observation_id, saved_url_hash, action (`report_unavailable`, `clear_report`), previous_report_id nullable, device_id, recorded_at, mutation_id.
  >
  > **ListingLinkReport — validation/lifecycle**: Append owner reports against the exact saved observation/link; enforce owner membership and mutation uniqueness. Display reporter/time; an API recheck cannot clear a report or certify the original page. No submitted URL is fetched.
...
- [ ] T041 [US2] Implement preview/hash approval, user-triggered search/recheck, one in-flight fetch and 30-second refresh spacing with separate source/local statuses and prior observations; append/clear owner link reports against saved observations without fetching their URLs. Files: `career_helper/application/jobs.py`. **Depends**: T040. **Basis**: FR-031–FR-035; API job-search-previews/job-searches/listings/recheck. **Done**: Changed external fields invalidate approval; source failure preserves existing records and manual routes.
...
- [ ] T043 [US2] Add protected search, listing/detail/recheck/link-report and target/requirement routes with paginated owner-only outcomes and safe original links. Files: `career_helper/web/listings.py`, `career_helper/web/targets.py`, `config/urls.py`. **Depends**: T042. **Basis**: FR-007, FR-031–FR-035; API Endpoint Catalogue. **Done**: Contract tests cover denial, validation and all search/target result shapes.

- [ ] T044 [US2] Build query preview, separate listing results/detail and manual-description/target requirement screens with source variants, freshness, original-link report/clear controls and empty/failure/manual fallback feedback; distinguish user reports from independent page checks. Files: `career_helper/templates/career_helper/listings.html`, `career_helper/templates/career_helper/target.html`, `career_helper/static/career_helper/listings.js`. **Depends**: T043. **Basis**: FR-007, FR-031–FR-035; UX 02A/12A. **Done**: Q5 browser path reaches a selected target without mislabelling directions or stale vacancies.

### Tests first

- [ ] T045 [US2] Author focused command tests for permission/coverage/endpoint evidence and refusal to certify fixture observations as live. Files: `tests/contract/test_source_qualification.py`. **Depends**: T044. **Basis**: FR-031; SC-011; Research R5; Q5. **Test contract**: Command assertions fail without the qualification implementation. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T046.
...
- [ ] T048 [P] [US3] Author focused contract/integration/browser tests for semantic clause review, unsupported quantities/entities, accept/reject, two-target history, stale dependencies and restoration. Include forward/reverse schema checks with populated predecessor fixtures and the phase's declared model constraints. Files: `tests/contract/test_resumes.py`, `tests/integration/test_resume_history.py`, `tests/e2e/preparation.spec.js`, `tests/migrations/test_preparation_schema.py`. **Depends**: T047. **Basis**: FR-002, FR-008–FR-010; SC-002; Q3/Q6. **Test contract**: A valid citation attached to an unsupported fact is rejected and historical review markers persist. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T051, T054, T057, T058.

- [ ] T049 [P] [US3] Author focused scheduler/progress tests for confirmed requirements, three task categories, omitted/malformed/past dates, zero/negative/non-integer budgets, month-end/timezone, shortfalls, manual override and preserved completion. Files: `tests/unit/test_preparation.py`, `tests/integration/test_task_progress.py`. **Depends**: T047. **Basis**: FR-011–FR-013; SC-003; Q6/Q7. **Test contract**: Tests cover input boundaries and ordered outcomes rather than repeating a ranking formula. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T052, T055, T056.

- [ ] T050 [P] [US3] Author focused coaching-boundary tests for finite operations, unknown fields/enums/citations, untrusted instructions, ungranted processing, new factual additions and stale publication. Files: `tests/contract/test_coaching.py`. **Depends**: T047. **Basis**: FR-009, FR-029, FR-030; External services Structured Coaching. **Test contract**: Request capture proves minimum scope and no hidden source-description transfer or tools. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T053, T071, T074, T085, T086.
...
- [ ] T063 [P] [US4] Author focused material import/quality/provenance tests for exact duplicates, conflicting/missing answers, unknown fields and embedded instructions without automatic URL fetching. Files: `tests/contract/test_materials.py`. **Depends**: T062. **Basis**: FR-014, FR-015, FR-029; SC-004; Q7. **Test contract**: Sources/independent flags survive import; quality changes append immutable revisions and invalidate dependent evidence; prompt/answer edits are rejected, with correction through a new import plus explicit doubt on unreliable old material. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T067, T070.

- [ ] T064 [P] [US4] Author focused interview/answer/hint tests for eligible frozen context, sourced-question quota with explicit no-applicable-external-material handling, both modes, empty-submit rejection, optional null-or-0–10 self-ratings, explicit submit, final confirmation and incomplete end states. Include forward/reverse schema checks with populated predecessor fixtures and the phase's declared model constraints. Files: `tests/contract/test_interviews.py`, `tests/unit/test_interviews.py`, `tests/migrations/test_interviews_schema.py`. **Depends**: T062. **Basis**: FR-016–FR-020, FR-023; Q8; API interview routes. **Test contract**: Before-submit follow-ups and real-mode hints fail; acoustic inference is excluded. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T067, T068, T069, T071, T074.

- [ ] T065 [P] [US4] Author focused recording/STT tests for trusted reservation deadlines, duplicate Done, partial uploads, expiry, early completion, purge failure, late retries and revoked grants. Files: `tests/integration/test_recordings.py`, `tests/contract/test_speech.py`. **Depends**: T062. **Basis**: FR-017, FR-020, FR-027, FR-030; Q8/Q11. **Test contract**: Actual files and request capture distinguish upload, submission, access revocation and physical removal. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T069, T072, T073.

- [ ] T066 [P] [US4] Author focused browser-rule/journey tests for silence, OS interruption, microphone denial, 64 MiB bound, failed upload, active expiry, suspend/resume, text fallback and local-only playback. Files: `tests/browser-unit/recorder.test.mjs`, `tests/browser-unit/playback.test.mjs`, `tests/e2e/interviews.spec.js`. **Depends**: T062. **Basis**: FR-017–FR-020, FR-027; Interaction lifecycle Manual-Turn Protocol/Recording Retention. **Test contract**: Unsaved memory differs from acknowledged data; verify pre-capture suspension disclosure, no persistent cache, acknowledgement/cancellation/teardown cleanup and expiry-before-action on resume. Tests do not certify memory clearing while suspended. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T076, T077, T078.

### Models and migrations

- [ ] T067 [US4] Add material identity/revision tables before their first interview consumer, preserving independent quality flags and exact source revisions. Files: `career_helper/models/materials.py`, `career_helper/models/__init__.py`, `career_helper/migrations/0010_materials.py`. **Depends**: T063, T064, T065, T066. **Basis**: FR-014–FR-016; Plan W5; Data model Materials, Interviews, and Recordings. **Done**: Prior plans survive migration; quality/tag updates never overwrite past question or answer text.

  Data-model constraints (verbatim; part of this task):

  > **LearningMaterial — fields**: owner, type question/reference, current_revision_id.
  >
  > **LearningMaterial — validation/lifecycle**: Stable material identity; quality/tag changes append a MaterialRevision. First-release content correction uses a new import and an explicit doubt flag on the old material; no in-place prompt/answer editor. Duplicate notice never discards provenance.
...
- [ ] T080 [P] [US5] Author focused review tests for four dimensions, exact final-answer spans, unknowns, malformed output, zero evidence, zero-to-three priorities and one-or-more retests. Include forward/reverse schema checks with populated predecessor fixtures and the phase's declared model constraints. Files: `tests/contract/test_reviews.py`, `tests/integration/test_review_evidence.py`, `tests/migrations/test_reviews_schema.py`. **Depends**: T079. **Basis**: FR-019, FR-021–FR-023; SC-007; Q9. **Test contract**: False citations, unconfirmed answers and acoustic inference are rejected. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T083, T084, T085.

- [ ] T081 [P] [US5] Author focused method lifecycle/journey tests for draft fields, same-revision completed retest, explicit confirmation, edit/new draft, withdrawal and evidence invalidation. Files: `tests/unit/test_methods.py`, `tests/integration/test_methods.py`, `tests/e2e/review_methods.spec.js`. **Depends**: T079. **Basis**: FR-024, FR-025; Q9. **Test contract**: Practice alone cannot promote a draft or carry verification to an edited revision. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T084, T086.

- [ ] T082 [P] [US5] Author focused tests for review-gap reprioritization, completed-task/manual-order preservation and transitive invalidation from corrected answers or substantive material quality changes and replacement imports with explicit doubt on old material. Files: `tests/integration/test_review_plan_invalidation.py`. **Depends**: T079. **Basis**: FR-002, FR-013, FR-019, FR-025; Q3/Q7. **Test contract**: Tests distinguish answer-source/doubt changes from label-only topic edits. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T087.
...
- [ ] T091 [P] [US6] Author focused two-device sync/conflict tests for missed cursors, reconnect, unsaved drafts, independent completions and stale writes after deletion. Files: `tests/integration/test_sync.py`, `tests/e2e/sync.spec.js`. **Depends**: T090. **Basis**: FR-026; SC-006; Q10. **Test contract**: Measure committed-change visibility and reject silent local-draft overwrites. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T094, T095.

- [ ] T092 [P] [US6] Author focused export/delete/policy tests across all FR-028 categories, archive authorization/expiry, copied dependant text, registered backups, retained audio and purge failures. Files: `tests/contract/test_privacy.py`, `tests/integration/test_exports_deletion.py`, `tests/e2e/privacy.spec.js`. **Depends**: T090. **Basis**: FR-027, FR-028; Q11/Q12; API privacy routes. **Test contract**: Manifest integrity and actual removal are distinct from access revocation. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T096, T097, T098, T099, T100.

- [ ] T093 [P] [US6] Author populated prior-schema migration and interrupted-upgrade/restore tests preserving IDs/order/provenance/review flags and reconciling deletion/expiry before private access; reject a pre-deletion backup after deletion even when purge fails, missing/corrupt current authority and unregistered/retired/tampered copies; exercise crashes before reconciliation and during replacement and ensure later accepted edits, rejected facts, revoked grants and changed credentials cannot be rolled back. Files: `tests/migrations/test_upgrade_restore.py`, `tests/fixtures/migrations/build_fixture.py`. **Depends**: T090. **Basis**: Constitution V; Plan Migrations and Compatibility; Q12. **Test contract**: Use actual earlier migrations, refuse newer schemas and avoid invented legacy import data. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T101.
...
- [ ] T098 [US6] Implement accepted-preview deletion, immediate revocation, cancelled/stale jobs, immediate restore-ineligibility of affected backups, registered-copy purge, content-free tombstones and dependant evidence-unavailable marking. Files: `career_helper/application/deletion.py`, `career_helper/application/evidence.py`. **Depends**: T097. **Basis**: FR-002, FR-028; Data model Concurrency, Processing Choice, and Privacy; Q12. **Done**: Delete-all covers copied text; late outputs cannot resurrect data and failed removal stays visibly incomplete.
...
- [ ] T101 [US6] Implement operator-only backup/restore maintenance and schema guards with registered backups, paused writers, policy-compliant copies and the intact current database as deletion authority, separately staged candidates and tombstone/expiry reconciliation before service resumes; preserve current authority/credentials/privacy choices through replacement, reject loss of later accepted records or current fact/conflict/review state, cancel replayable jobs and invalidate sessions/cursors. Missing authority or interrupted reconciliation keeps startup in maintenance; archive-only disaster recovery is unsupported. Files: `career_helper/management/commands/backup_private.py`, `career_helper/management/commands/restore_private.py`, `career_helper/application/maintenance.py`, `specs/001-career-interview-coach/operations.md`. **Depends**: T100. **Basis**: Constitution V; Plan Migrations and Compatibility; Q12. **Done**: Upgrade/restore tests pass without adding generic user archive import or temporary-audio backups.

### Tests first

- [ ] T102 [US6] Author focused audit-command tests against real temporary copies for expired, unknown, failed and physically removed states. Files: `tests/integration/test_retention_audit.py`. **Depends**: T101. **Basis**: FR-027, FR-028; Quickstart audit_retention; Q11. **Test contract**: Inaccessible copies cannot pass the physical-removal assertion. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T103.
...
- [ ] T109 Revalidate the one configured job source through the qualification command, current permission basis, non-personal request preview and observed mainland workplace content. Files: `specs/001-career-interview-coach/evidence/job-source-live.md`. **Depends**: T047. **Basis**: FR-031–FR-035; SC-011, SC-012; Research R5; Q5. **Done**: Record actual endpoint/time/counts/IDs and limitations; do not reuse historical research counts or claim broad market coverage.

### Live configuration decision

- [ ] T110 Qualify candidate coaching and STT configurations against current terms, region/access, structured Chinese output, actual formats and recording-copy deletion; document the chosen minimum configuration and disclosure. Files: `specs/001-career-interview-coach/evidence/training-configuration.md`, `specs/001-career-interview-coach/contracts/external-services.md`. **Depends**: T061, T079, T090. **Basis**: FR-017, FR-027, FR-030; Research R6; External services Live Training Qualification Gate. **Done**: No personal payload or live enablement yet; missing owner-supplied configuration/permission evidence or incompatible retention is an explicit gate, not an invented provider.

### Tests first

- [ ] T111 [P] Author contract tests for the selected coaching transport using recorded synthetic response shapes and a local request recorder, including limits, grants and safe failures. Files: `tests/contract/test_coaching_service.py`. **Depends**: T110. **Basis**: FR-009, FR-021, FR-029, FR-030; External services Structured Coaching. **Test contract**: Tests fail until the selected concrete callable satisfies existing operation contracts. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T113.

- [ ] T112 [P] Author contract tests for the selected STT transport with WebM/Opus and MP4/AAC inputs, grant/expiry guards, transient failures and provider-copy lifecycle evidence requirements. Files: `tests/contract/test_speech_service.py`. **Depends**: T110. **Basis**: FR-017, FR-019, FR-020, FR-027, FR-030; External services Speech Input and Output. **Test contract**: Tests fail until the selected callable satisfies MIME/privacy/lifetime boundaries. **Done**: Assertions/fixtures reviewed; record runnable RED or the missing structural prerequisite. **RED owners**: T114.
...
- [ ] T116 On an authorized qualification host, exercise independent cleanup with stopped web/jobs, watchdog/overdue detection, restart reconciliation and the complete backup/provider-copy inventory. Files: `specs/001-career-interview-coach/evidence/hosting-retention.md`. **Depends**: T107, T115. **Basis**: FR-027, FR-028; Research R8; Q11. **Done**: Record physical deletion evidence and host-wide failure limits; an incompatible host/provider stays unqualified for live recording.

### Physical-device verification

- [ ] T117 Run real desktop Chrome/Edge/Safari and physical iOS Safari/Android Chrome voice paths, including manual turns, silence, hints, corrected terminology, saved review, text fallback and suspend/expiry handling. Files: `specs/001-career-interview-coach/evidence/devices-voice.md`. **Depends**: T116. **Basis**: FR-017–FR-020, FR-027; SC-005; Q8/Q11. **Done**: Desktop and phone each complete at least 10 minutes with actual audio. Verify active expiry, acknowledgement/cancellation buffer cleanup, no persistent cache, pre-capture suspension disclosure and clearing expired buffers before any audio action on resume. Emulation cannot replace this evidence or prove local playback support; memory clearing during suspension is explicitly outside the approved guarantee.
...
flowchart LR
  ...
  U2 --> J[Live source qualification]
  U5 --> L[Training configuration and qualification]
  R --> P[Deployment preparation]
  R --> V[Visual and accessibility evidence]
  P --> D[Hosting and physical devices]
  L --> D
...
4. Complete offline regression, then qualified live services, hosting, actual devices, visual/accessibility review and timed outcomes. A missing resource does not prevent independent work. T109 source qualification depends on the implemented source command/offline evidence; T110 training qualification depends on its implemented command and consumers, not Figma or deployment. T116 still requires deployment preparation and qualified transports; T121 joins all mandatory evidence. An unavailable gate stays unchecked.
